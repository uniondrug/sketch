...
class AppServiceProvider implements ServiceProviderInterface
{
    ...
    {
        $iterator = new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($di->appPath().DIRECTORY_SEPARATOR.'Services'), \RecursiveIteratorIterator::SELF_FIRST);
        foreach ($iterator as $item) {
            if (Text::endsWith($item, 'Service.php', false)) {
                $name = str_replace([
                    $di->appPath().DIRECTORY_SEPARATOR.'Services'.DIRECTORY_SEPARATOR,
                    '.php'
                ], '', $item);
                if ($name) {
